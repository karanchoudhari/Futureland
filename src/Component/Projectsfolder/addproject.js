...
// import { Plus, Upload, X, CheckCircle, Trash, Edit } from "lucide-react"; // Added Edit icon
...
//   const [editingIndex, setEditingIndex] = useState(null); // Track the index of the project being edited
...
//       updatedProjectList[editingIndex] = formData;
...
const ProjectForm = () => {
  ...
  //         {}, // Empty body since you're not sending any data in the body
  //         {
  //           headers: {
  ...
  //         }
  //       );
  //       console.log('Projects:', response.data); // Update URL if needed
  ...
  const handleSubmit = async(e) => {
    ...
    try {
      if (editingIndex !== null) {
        // Update existing project
        await axios.put(`http://localhost:3002/api/projects/${editingIndex}`, formData);
        const updatedProjectList = [...projectList];
        updatedProjectList[editingIndex] = formData;
        setProjectList(updatedProjectList);
...
//   const [projectList, setProjectList] = useState([]); // Initialize as empty array
//   const [sectors, setSectors] = useState(["Construction", "Retail", "Infrastructure", "Education", "Commercial"]);
//   const [newSector, setNewSector] = useState("");
//   const [showModal, setShowModal] = useState(false);
//   const [showSuccessModal, setShowSuccessModal] = useState(false);
...
//         });
//         console.log('Projects:', response.data);
//         setProjectList(response.data); // Ensure response.data is an array
//       } catch (error) {
//         console.error('Error fetching projects:', error);
//       }
//     };
...
//       } else {
//         // Add new project
//         const response = await axios.post('http://localhost:3002/api/projects', formData);
//         setProjectList((prev) => [...prev, response.data.project]); // Ensure projectList is updated correctly
//         setShowSuccessModal(true);
//       }
//     } catch (error) {
//       console.error('Error submitting project:', error);
...
// import { Country, State, City } from "country-state-city";
// import ProjectList from "./projectlist";
// import { Plus, Upload, X, CheckCircle, Trash, Edit } from "lucide-react";
// import { useNavigate } from "react-router-dom";
// import { TextField, MenuItem, Button, Box, Typography, Grid, Paper, Divider, IconButton } from '@mui/material';
...
//     const emptyFields = Object.entries(formData)
//       .filter(([key, value]) => {
//         if (key === "start" || key === "finish") return value === null;
//         return value === "";
//       })
//       .map(([key]) => key);
...
//       if (key === "start" || key === "finish") {
//         submissionData.append(key, value ? value.toISOString() : "");
//       } else if (key === "kmlFile" && value) {
...
//       if (editingIndex !== null) {
//         // Update existing project
//         await axios.put(`http://localhost:3002/api/project/${editingIndex}`, submissionData, {
...
//       } else {
//         // Add new project
//         const response = await axios.post('http://localhost:3002/api/project', submissionData, {
//           headers: { 'Content-Type': 'multipart/form-data' }
//         });
//         setProjectList([...projectList, response.data]);
//         setShowSuccessModal(true);
