...
const MapComponent = ({ kmlColor, kmlOpacity, projectid }) => {
  ...
  useEffect(() => {
    try {
      ...
      setError("Failed to load KML data. Please try again.");
    }
  }, [dispatch, projectid]);
  ...
  const fetchAndConvertKML = async (url) => {
    try {
      const response = await fetch(url);
      if (!response.ok) throw new Error(`Failed to fetch KML: ${response.statusText}`);
      const blob = await response.blob();
      ...
      if (url.endsWith(".kmz")) {
        const zip = await JSZip.loadAsync(blob);
        const kmlFileName = Object.keys(zip.files).find((file) => file.endsWith(".kml"));
        if (!kmlFileName) throw new Error("No KML file found in KMZ.");
      ...
      const geoJson = kml(kmlData);
      if (!geoJson || !geoJson.features.length) throw new Error("KML conversion failed or empty data.");
    ...
    if (kmlData?.kml?.length > 0) {
      const fetchAllKMLs = async () => {
        try {
          ...
          // Adjust view to the first feature
          if (validGeoJsons.length > 0 && validGeoJsons[0]?.features.length > 0) {
            const firstFeature = validGeoJsons[0].features[0];
            let newLatitude, newLongitude;
            ...
            if (newLatitude !== undefined && newLongitude !== undefined && mapRef.current) {
              mapRef.current.easeTo({
                center: [newLongitude, newLatitude],
                zoom: 10,
                duration: 3000,
        ...
        {multipleGeoJson.map((geoJson, index) => (
          <Source key={`source-${index}`} id={`source-${index}`} type="geojson" data={geoJson}>
            {geoJson.features.map((feature, idx) => {
              ...
              switch (feature.geometry.type) {
                case "LineString":
                  layerProps = { type: "line", paint: { "line-color": "#ff0000", "line-width": 3, "line-opacity": 1 } };
                  break;
                case "Polygon":
                case "MultiPolygon":
                  layerProps = { type: "fill", paint: { "fill-color": kmlColor || "#0000ff", "fill-opacity": kmlOpacity || 0.4 } };
                  break;
                case "Point":
                  layerProps = { type: "circle", paint: { "circle-radius": 6, "circle-color": "#ff0000" } };
                  break;
              }
