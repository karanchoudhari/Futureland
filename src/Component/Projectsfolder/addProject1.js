...
const AddProject1 = ({ editingProject, onClose }) => {
  ...
  return (
    ...
    <Paper elevation={3} sx={{ p: 3, borderRadius: 4, maxWidth: 750, width: '100%', background: '#fff', mt: 0, position: 'relative' }}>
      ...
      {/* Add Sector Modal */}
      {showModal && (
        <div className="fixed inset-0 black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-lg p-6 w-80 relative" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setShowModal(false)} className="absolute top-1 right-1 p-1 hover:bg-gray-100 rounded-full">
              <X className="w-5 h-5" />
            </button>
